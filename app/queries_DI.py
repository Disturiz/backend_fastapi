...
MONTHLY_SQL = f"""
SELECT
    VARCHAR_FORMAT({SAFE_DATE_EXPR}, 'YYYY-MM') AS month,
    COALESCE(SUM(total_rev), 0) AS revenue,
    COUNT(DISTINCT order_id) AS orders,
    COUNT(DISTINCT cust_email) AS customers
FROM {TARGET_FULL}
{BASE_FILTER}
GROUP BY VARCHAR_FORMAT({SAFE_DATE_EXPR}, 'YYYY-MM')
ORDER BY month
"""
...
cohort_base AS (
    SELECT
        ...
        DATE(
            VARCHAR_FORMAT(
                MIN(safe_date) OVER (PARTITION BY cust_email),
                'YYYY-MM'
            ) || '-01'
        ) AS cohort_month,
        DATE(
            VARCHAR_FORMAT(safe_date, 'YYYY-MM') || '-01'
        ) AS activity_month
    FROM filtered_sales
),
cohort_activity AS (
    SELECT
        ...
        activity_month,
        (
            (YEAR(activity_month) - YEAR(cohort_month)) * 12
...
SELECT
    VARCHAR_FORMAT(a.cohort_month, 'YYYY-MM') AS cohort_month,
    a.month_index,
    a.active_customers,
    s.cohort_size,
    DECIMAL(
        a.active_customers / NULLIF(s.cohort_size, 0),
        15,
        4
    ) AS retention_pct
...
WITH ordered_sales AS (
    SELECT
        ...
        LEAD(product) OVER (
            PARTITION BY cust_email
            ORDER BY {SAFE_DATE_EXPR}, order_id
        ) AS target_product
    FROM {TARGET_FULL}
)
SELECT
